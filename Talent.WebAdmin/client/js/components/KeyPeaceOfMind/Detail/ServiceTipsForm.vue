<template>
  <div class="row px-4 py-4 w-100">
              ...
              <div class="input-group talent_front">
                ...
                <div class="d-flex align-items-center w-100">
                  <multiselect
                    ...
                    label="serviceTipMenuName"
                    track-by="serviceTipMenuId"
              ...
              <div class="input-group talent_front">
                ...
                <div class="d-flex align-items-center w-100">
                  <multiselect
                    ...
                    name="Category"
                    :options="categories"
                    label="serviceTipTypeName"
                    track-by="serviceTipTypeId"
      ...
      <div class="col-md-12 talent_magin_small mt-3">
          <div class="row align-items-center justify-content-between">
              <div class="col d-flex py-0 pl-0 pr-4">
                  ...
                  <div class="input-group">
                      <input class="form-control" placeholder="Search By Menu" v-model="params.ServiceTipMenuName" @keyup.enter="handleFilter"/>
                  </div>
              </div>
          ...
          <table class="table table-bordered table-responsive-md talent_table_padding">
              ...
              <tbody>
                  <tr v-for="dictionary in productTypes" :key="dictionary.productTypeId">
                      <td>
                          {{dictionary.serviceTipMenuName}}
                      </td>
                      <td>
                          {{dictionary.cms_MenuName}}
                      </td>
                      <td>
                          {{dictionary.serviceTipTypeName}}
                      </td>
                      <td>
    ...
    <modal-service-menu-type
      name="modalKodawariType"/>

    <modal-cms-content-full-service-tips
...
import ModalCmsContentFull from '../../ProductInformation/ModalCmsContentFull.vue';
...
import ModalServiceMenuType from '../../ProductInformation/ModalServiceMenuType.vue';
import ModalServiceMenu from '../../ProductInformation/ModalServiceMenu.vue';
import ModalCmsContentFullServiceTips from '../../ProductInformation/ModalCmsContentFullServiceTips.vue';

export default {
  components: { ModalDelete, ModalCmsMenu, ModalSpwaCategory, ModalCmsContentFull, ModalCmsSubContent, ModalKodawariMenu, ModalKodawariType, ModalServiceMenuType, ModalServiceMenu, ModalCmsContentFullServiceTips },
    async mounted() {
    this.isBusy = true
      try {
    ...
    methods: {
      ...
      async fetchKodawariMenu() {
        try {
          const res = await this.service.getAllServiceTipsMenu({
            PageIndex: 1,
            PageSize: 100
          })
          this.kodawariMenus = res.serviceTipMenus
        } catch(err) {
          console.error(err)
        }
      },
      ...
      async fetchCmsContent() {
        try {
          const res = await this.service.getAllCMSContent({
            Cms_ContentCategory: 'ServiceTips',
            PageIndex: 1,
            PageSize: 100
          })
      ...
      async fetchCmsSubContent() {
        try {
          const res = await this.service.getAllCMSSubContent({
            Cms_SubContentCategory: 'ServiceTips',
            PageIndex: 1,
            PageSize: 100
          })
      ...
      async fetchCategories() {
        try {
          const res = await this.service.getAllServiceTipsType(this.paramsCategories)
          this.categories = res.serviceTipTypes
        } catch(err) {
          console.error(err)
        }
      },
      async fetchFAQ() {
        ...
        try {
          const res = await this.service.getAllServiceTips(this.params)
          this.productTypes = res.serviceTips
          this.totalData = res.totalServiceTips
        } catch(err) {
          console.error(err)
        }
      ...
      async submit(e) {
        ...
        try {
          const body = {
            "serviceTipTypeId": this.selectedCategory.serviceTipTypeId,
            ...
            "serviceTipMenuId": this.selectedKodawariMenu.serviceTipMenuId,
            approvedAt: e ?moment().format('YYYY-MM-DD')  : null
          }
          if(!this.selectedEditData) {
            await this.service.createServiceTips(body)
          } else {
            body.serviceTipId = this.selectedEditData.serviceTipId
            await this.service.updateServiceTips(body)
          }
      ...
      async handleDelete() {
        this.isBusy = true
        try {
          await this.service.deleteServiceTips(this.selectedData.serviceTipId)
          this.fetchFAQ()
          this.success = true
          this.sequence = 1
          this.topic = ''
      ...
      handleEdit(data) {
        ...
        this.selectedCategory = {
          serviceTipTypeId: data.serviceTipTypeId,
          serviceTipTypeName: data.serviceTipTypeName
        }
        ...
        this.selectedKodawariMenu = {
          serviceTipMenuId: data.serviceTipMenuId,
          serviceTipMenuName: data.serviceTipMenuName
        }
      ...
      async handleEditStatus(data) {
        this.isBusy = true
        await this.service.updateServiceTips({
          serviceTipId: data.serviceTipId,
          "serviceTipTypeId": data.serviceTipTypeId,
