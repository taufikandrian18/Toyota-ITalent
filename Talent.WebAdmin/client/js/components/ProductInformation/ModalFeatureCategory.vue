<template>
    <div class="modal fade" :id="name" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="row px-4 py-4">
                  ...
                  <div class="col-md-6">
                    <div class="row">
                      <div class="col-md-12">
                        <label>Product Feature Category Name<span class="talent_font_red"> *</span></label>
                ...
                <div class="col-md-12 talent_magin_small mt-3">
                    <div class="row align-items-center justify-content-between">
                        <div class="col d-flex py-0 pl-0 pr-4">
                            ...
                            <div class="input-group">
                                <input class="form-control" placeholder="Search By Name" v-model="params.ProductFeatureCategoryName" @keyup.enter="handleFilter"/>
                            </div>
                        </div>
                ...
                <div class="w-100 talent_overflowx mt-3">
                    <table class="table table-bordered table-responsive-md talent_table_padding">
                        ...
                        <tbody>
                            <tr v-for="dictionary in categories" :key="dictionary.productFeatureCategoryId">
                                <td>
                                ...
                                <td class="talent_nopadding_important">
                                    <button type="button" class="btn btn-block talent_bg_red talent_font_white" 
                                            data-toggle="modal"
                                            data-target="#modalDeleteFeatureCategory" @click="selectedData = dictionary">
                                    Remove
                                    </button>
                                </td>
                            </tr>
        ...
        <modal-delete 
          name="modalDeleteFeatureCategory"
          @delete="handleDelete"
        />
    </div>
</template>
  ...
  components: { ModalDelete },
    ...
    methods: {
      async fetchCategories() {
        ...
        try {
            const res = await this.service.getAllProductFeatureCategories(this.params)
            this.categories = res.productFeatureCategories
            this.totalData = res.totalProductFeatureCategories
        } catch (err) {
            console.error(err)
        }
      },
      async handleSave() {
        ...
        try {
          if(this.selectedEditData) {
            const body = {
              productFeatureCategoryId: this.selectedEditData.productFeatureCategoryId,
              productId: this.id,
              "productFeatureCategoryName": this.categoryName
            }
            if(this.image.base64) {
              body.productFeatureFileContent = {
                base64: this.image.base64,
                fileName: this.image.fileName,
                contentType: this.image.contentType
              }
            }
            const res = await this.service.updateProductFeatureCategories(body)
            ...
            const res = await this.service.createProductFeatureCategories({
              productId: this.id,
              "productFeatureCategoryName": this.categoryName,
              productFeatureFileContent: {
                base64: this.image.base64,
                fileName: this.image.fileName,
                contentType: this.image.contentType
              },
      ...
      async handleDelete() {
        this.isBusy = true
        try {
          await this.service.deleteProductFeatureCategories(this.selectedData.productFeatureCategoryId)
          this.fetchCategories()
          this.success = true
          this.categoryName = ''
          vm.image= {
      ...
      handleEdit(category) {
        ...
        this.categoryName = category.productFeatureCategoryName
