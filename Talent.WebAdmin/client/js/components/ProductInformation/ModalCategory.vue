            ...
            <div class="modal-content">
                ...
                <div class="col-md-12 talent_magin_small mt-3">
                    <div class="row align-items-center justify-content-between">
                        <div class="col d-flex py-0 pl-0 pr-4">
                            ...
                            <div class="input-group">
                                <input class="form-control" placeholder="Search By Name" v-model="params.ProductCategoryName"/>
                            </div>
                        </div>
                ...
                <div class="w-100 talent_overflowx mt-3">
                    <table class="table table-bordered table-responsive-md talent_table_padding">
                        ...
                        <tbody>
                            <tr v-for="dictionary in categories" :key="dictionary.productCategoryId">
                                <td>
                                    {{dictionary.productCategoryName}}
                                </td>
                ...
                <!-- <div class="modal-footer">
                    <div class="col-md-12 d-flex justify-content-center">
                        <div class="col-md-12 d-flex justify-content-around">
                            <button class="btn talent_bg_red talent_font_white talent_roundborder" type="button" @click="handleSave">Save</button>
  ...
  components: { ModalDelete },
    ...
    methods: {
      async fetchCategories() {
        try {
            const res = await this.service.getAllCategories(this.params)
            this.categories = res.productCategories
            this.totalData = res.totalProductCategories
        } catch (err) {
            console.error(err)
        }
      },
        ...
        try {
          if(this.selectedEditData) {
            await this.service.updateProductCategory({
              productCategoryId: this.selectedEditData.productCategoryId,
              "productCategoryName": this.categoryName
            })

          } else {
            await this.service.createProductCategory({
              "productCategoryName": this.categoryName
            })
          }
          this.categoryName = ''
      ...
      async handleDelete() {
        this.isBusy = true
        try {
          await this.service.deleteProductCategory(this.selectedData.productCategoryId)
          this.fetchCategories()
          this.success = true
          this.categoryName = ''
        } catch (err) {
      ...
      handleEdit(category) {
        ...
        this.categoryName = category.productCategoryName
      }
    }
}
</script>
