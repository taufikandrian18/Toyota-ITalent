            ...
            <div class="modal-content">
                ...
                <div class="col-md-12 talent_magin_small mt-3">
                    <div class="row align-items-center justify-content-between">
                        <div class="col d-flex py-0 pl-0 pr-4">
                            ...
                            <div class="input-group">
                                <input class="form-control" placeholder="Search By Name" v-model="params.KeyPieceOfMindMenuName"/>
                            </div>
                        </div>
                ...
                <div class="w-100 talent_overflowx mt-3">
                    <table class="table table-bordered table-responsive-md talent_table_padding">
                        ...
                        <tbody>
                            <tr v-for="dictionary in categories" :key="dictionary.keyPieceOfMindMenuId">
                                <td>
  ...
  components: { ModalDelete },
    ...
    methods: {
      async fetchCategories() {
        this.params.Cms_MenuCategory = this.type
        try {
            const res = await this.service.getAllKPOMMenu(this.params)
            this.categories = res.keyPieceOfMindMenus
            this.totalData = res.totalKeyPieceOfMindMenus
        } catch (err) {
            console.error(err)
        }
      },
      async handleSave() {
        ...
        try {
          if(this.selectedEditData) {
            await this.service.updateKPOMMenu({
              keyPieceOfMindMenuId: this.selectedEditData.keyPieceOfMindMenuId,
              "keyPieceOfMindMenuName": this.categoryName,
              "keyPieceOfMindMenuSequence": 0
            })

          } else {
            await this.service.createKPOMMenu({
              "keyPieceOfMindMenuName": this.categoryName,
              "keyPieceOfMindMenuSequence": 0
            })
          }
      ...
      async handleDelete() {
        this.isBusy = true
        try {
          await this.service.deleteKPOMMenu(this.selectedData.keyPieceOfMindMenuId)
          this.fetchCategories()
          this.success = true
          this.categoryName = ''
        } catch (err) {
      ...
      handleEdit(category) {
        ...
        this.categoryName = category.keyPieceOfMindMenuName
      }
    }
}
</script>
