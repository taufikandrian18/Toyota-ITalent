            ...
            <div class="modal-content">
                ...
                <div class="row px-4 py-4">
                  ...
                  <div class="col-md-6">
                    <div class="row">
                      <div class="col-md-12">
                        <label>SPWA Category Name<span class="talent_font_red"> *</span></label>
                ...
                <div class="col-md-12 talent_magin_small mt-3">
                    <div class="row align-items-center justify-content-between">
                        <div class="col d-flex py-0 pl-0 pr-4">
                            ...
                            <div class="input-group">
                                <input class="form-control" placeholder="Search By Name" v-model="params.productSPWACategoryName"/>
                            </div>
                        </div>
                ...
                <div class="w-100 talent_overflowx mt-3">
                    <table class="table table-bordered table-responsive-md talent_table_padding">
                        ...
                        <tbody>
                            <tr v-for="dictionary in categories" :key="dictionary.productSPWACategoryId">
                                <td>
                                ...
                                <td class="talent_nopadding_important">
                                    <button type="button" class="btn btn-block talent_bg_red talent_font_white" 
                                            data-toggle="modal"
                                            data-target="#modalDeleteFeatureSPWA" @click="selectedData = dictionary">
                                    Remove
                                    </button>
                                </td>
                            </tr>
  ...
  components: { ModalDelete },
    ...
    methods: {
      async fetchCategories() {
        ...
        try {
            const res = await this.service.getAllSPWACategory(this.params)
            this.categories = res.productSPWACategories
            this.totalData = res.totalProductSPWACategories
        } catch (err) {
            console.error(err)
        }
      },
      async handleSave() {
        ...
        try {
          if(this.selectedEditData) {
            const body = {
              productId: this.id,
              productSPWACategoryId: this.selectedEditData.productSPWACategoryId,
              "productSPWACategoryName": this.categoryName
            }
            if(this.image.base64) {
              body.productFileContent = {
                base64: this.image.base64,
                fileName: this.image.fileName,
                contentType: this.image.contentType
              }
            }
            await this.service.updateSPWACategory(body)

          } else {
            await this.service.createSPWACategory({
              productId: this.id,
              "productSPWACategoryName": this.categoryName,
              productFileContent: {
                base64: this.image.base64,
      ...
      async handleDelete() {
        this.isBusy = true
        try {
          await this.service.deleteSPWACategory(this.selectedData.productSPWACategoryId)
          this.fetchCategories()
          this.success = true
          this.categoryName = ''
          vm.image= {
      ...
      handleEdit(category) {
        ...
        this.categoryName = category.productSPWACategoryName
