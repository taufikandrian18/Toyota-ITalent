<template>
    <div class="modal fade" :id="name" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                        ...
                        <div class="input-group talent_front">
                          ...
                          <div class="d-flex align-items-center w-100">
                            <div class="">
                              ...
                              <!-- </div> -->
                                <input type="file" :name="`input-manfaat-${this.name}`" :id="`input-manfaat-${this.name}`" style="display: none;" @change="handleChangeFile" accept=".png,.jpg,.jpeg">
                            </div>
                          </div>
                        </div>
                      </div>
                            ...
                            <div class="">
                              ...
                              <div class="btn-upload">
                                <button class="btn btn-primary">
                                  <label for="input-manfaat-video" style="margin: 0px">Browse</label>
                                </button>
                              </div>
                                <input type="file" name="input-manfaat-video" id="input-manfaat-video" style="display: none;" @change="handleChangeFileVideo" accept=".mp4">
                            </div>
                          </div>
                        </div>
                      </div>
                            ...
                            <tr v-for="dictionary in categories" :key="dictionary.cms_ContentId">
                                ...
                                <td class="talent_nopadding_important">
                                    <button type="button" class="btn btn-block talent_bg_red talent_font_white" 
                                            data-toggle="modal"
                                            data-target="#modalDeleteCmsContentFullTestDrive" @click="selectedData = dictionary">
                                    Remove
                                    </button>
                                </td>
                            </tr>
        ...
        <modal-delete 
          name="modalDeleteCmsContentFullTestDrive"
          @delete="handleDelete"
        />
    </div>
</template>
  ...
  components: { ModalDelete },
    ...
    methods: {
          ...
          if(this.selectedEditData) {
            const body = {
              ...
              "cmsContentVidContent": {
                "base64": this.imageVideo.contentType == 'mp4' ? this.imageVideo.base64 : '',
                "fileName": this.imageVideo.contentType == 'mp4' ? this.imageVideo.fileName : '',
                "contentType": this.imageVideo.contentType == 'mp4' ? this.imageVideo.contentType : ''
              },
      ...
      async handleChangeFileVideo(e) {
        ...
        this.imageVideo.base64 = res
        this.imageVideo.fileName = selectedImage.name
        this.imageVideo.contentType = extension
      },
      getBase64Video(file) {
        return new Promise((resolve, reject) => {
            const reader = new FileReader();
