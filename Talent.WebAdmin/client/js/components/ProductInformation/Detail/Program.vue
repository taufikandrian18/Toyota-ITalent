<template>
  <div class="row px-4 py-4">
    ...
    <div class="col-md-12 mb-3">
      <h5>Non Product Benefit & Program</h5>
    </div>
    <div class="col-md-6">
      <div class="row">
        <!-- <div class="col-md-12">
          ...
          <div class="input-group talent_front">
            ...
            <div class="d-flex align-items-center w-100">
              <multiselect
                ...
                name="Category"
                :options="categories"
                label="productProgramCategoryName"
                track-by="productProgramCategoryId"
      ...
      <div class="col-md-12 talent_magin_small mt-3">
          <div class="row align-items-center justify-content-between">
              <div class="col d-flex py-0 pl-0 pr-4">
                  ...
                  <div class="input-group">
                      <input class="form-control" placeholder="Search By Content" v-model="params.Query" @keyup.enter="handleFilter"/>
                  </div>
              </div>
          ...
          <table class="table table-bordered table-responsive-md talent_table_padding">
              ...
              <tbody>
                  <tr v-for="dictionary in productTypes" :key="dictionary.productTypeId">
                      <td>
                          {{dictionary.productProgramCategoryName}}
                      </td>
                      <td>
...
import ModalCmsContentFull from '../ModalCmsContentFull.vue';
import ModalProgramCategory from '../ModalProgramCategory.vue';

export default {
  components: { ModalDelete, ModalCmsMenu, ModalSpwaCategory, ModalCmsContentFull, ModalProgramCategory },
    async mounted() {
      this.isBusy = true
      try {
      ...
      $('#modalProgramCategory').on('hidden.bs.modal', function (e) {
        vm.fetchCategories()
      })
    },
    data() {
    ...
    methods: {
      ...
      handleAddSubCategory() {
        $('#modalProgramCategory').modal('show')
      },
      async fetchCmsMenu() {
        try {
          const res = await this.service.getAllCMSMenu({
            cms_MenuCategory: 'Program',
            PageIndex: 1,
            PageSize: 100
          })
      ...
      async fetchCmsContent() {
        try {
          const res = await this.service.getAllCMSContent(this.paramsCategories)
      ...
      async fetchCategories() {
        try {
          const res = await this.service.getAllProductProgramCategory({
          ProductId: this.id,
          PageIndex: 1,
          PageSize: 100
        })
          this.categories = res.productProgramCategories
        } catch(err) {
          console.error(err)
        }
      },
      async fetchFAQ() {
        try {
          ...
          const res = await this.service.getAllProductProgramMapping(this.params)
          this.productTypes = res.productProgramMappings
          this.totalData = res.totalProductProgramMapping
        } catch(err) {
          console.error(err)
        }
      },
      ...
      async submit(e) {
        ...
        try {
          const body = {
            productId: this.id,
            "productProgramCategoryId": this.selectedCategory.productProgramCategoryId,
          ...
          if(!this.selectedEditData) {
            await this.service.createProductProgramMapping(body)
          } else {
            body.productProgramMappingId = this.selectedEditData.productProgramMappingId
            await this.service.updateProductProgramMapping(body)
          }
      ...
      async handleDelete() {
        this.isBusy = true
        try {
          await this.service.deleteProductProgramMapping(this.selectedData.productProgramMappingId)
          this.fetchFAQ()
          this.success = true
          this.sequence = 1
          this.topic = ''
      ...
      handleEdit(data) {
        ...
        this.selectedCategory = {
          productProgramCategoryId: data.productProgramCategoryId,
          productProgramCategoryName: data.productProgramCategoryName
        }
      ...
      async handleEditStatus(data) {
        this.isBusy = true
        await this.service.updateProductProgramMapping({
          productProgramMappingId: data.productProgramMappingId,
          productId: this.id,
          "productProgramCategoryId": data.productProgramCategoryId,
