<template>
  <div class="row px-4 py-4 w-100">
    <loading-overlay :loading="isBusy"></loading-overlay>
    <div class="col-md-12 p-0 mb-4"  v-if="errored.length > 0">
      <div class="alert alert-danger alert-dismissible fade show" role="alert" v-if="errored.length > 0">
          <ul>
            <li v-for="error in errored" :key="error.key">{{error.value}}</li>
          </ul>
      </div>
    </div>
    <div class="col-md-12 p-0 mb-4"  v-if="success">
      <div class="alert alert-success alert-dismissible fade show" role="alert">
        Success
      </div>
    </div>
    <div class="col-md-12 mb-3">
      <h5>Customer Profile</h5>
    </div>
    <div class="col-md-6">
      <div class="row">
        <div class="col-md-12">
          <div class="input-group talent_front">
            <label for="" class="w-100">Choose Customer Type<span class="talent_font_red"> *</span></label>
            <div class="d-flex align-items-center w-100">
              <multiselect
                class="w-100 mr-2"
                name="Category"
                :options="categories"
                label="productCustomerTypeName"
                track-by="productCustomerTypeId"
                v-model="selectedCategory">
              </multiselect>
              <button class="btn_add_rounded" @click="handleAddSub">
                +
              </button>
            </div>
          </div>
        </div>
        <div class="col-md-12 mt-4">
          <label>Status<span class="talent_font_red"> *</span></label>
          <div class="input-group">
              <input name="status" type="text" class="form-control" v-model="status"/>
          </div>
        </div>
        <div class="col-md-12 mt-4">
          <label>Penghasilan</label>
          <div class="input-group">
              <input name="penghasilan" type="text" class="form-control" v-model="penghasilan"/>
          </div>
        </div>
        <div class="col-md-12 mt-4">
          <label>Kebutuhan</label>
          <div class="input-group">
              <input name="kebutuhan" type="text" class="form-control" v-model="kebutuhan"/>
          </div>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="row">
        <div class="col-md-12">
          <label>Umur<span class="talent_font_red"> *</span></label>
          <div class="input-group">
              <input name="umur" type="text" class="form-control" v-model="umur"/>
          </div>
        </div>
        <div class="col-md-12 mt-4">
          <label>Pekerjaan<span class="talent_font_red"> *</span></label>
          <div class="input-group">
              <input name="pekerjaan" type="text" class="form-control" v-model="pekerjaan"/>
          </div>
        </div>
        <div class="col-md-12 mt-4">
          <label>Kendaraan Sebelumnya<span class="talent_font_red"> *</span></label>
          <div class="input-group">
              <input name="kendaraanSebelumnya" type="text" class="form-control" v-model="kendaraanSebelumnya"/>
          </div>
        </div>
        <div class="col-md-12 mt-4">
          <label>Sumber Prospek</label>
          <div class="input-group">
              <input name="sumberProspek" type="text" class="form-control" v-model="sumberProspek"/>
          </div>
        </div>
      </div>
    </div>
    <div class="col-md-12 d-flex justify-content-end mt-4">
      <button class="btn btn-secondary mr-2" @click="reset">
          Close
      </button>
      <button class="btn btn-success mr-2" @click="submit(null)">
          {{ !selectedEditData ? 'Save' : 'Save'}}
      </button>
      <button class="btn btn-primary" @click="submit(1)">
          Submit
      </button>
    </div>

    <!-- divider -->
    <div class="col-md-12 py-2">
      <hr>
    </div>

    <!-- table data -->
    <div class="col-md-12">
      <!-- filter -->
      <div class="col-md-12 talent_magin_small mt-3">
          <!-- <div class="row align-items-center justify-content-between">
              <div class="col d-flex py-0 pl-0 pr-4">
                  <div class="dropdown">
                    <button class="btn btn-info mr-2" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-expanded="false" @click="handleFilter">
                      Filter
                    </button>
                  </div>
                  <div class="input-group">
                      <input class="form-control" placeholder="Search By Name" v-model="params.ProductTypeName"/>
                  </div>
              </div>
              <div class="col-8 row d-flex justify-content-end">
              </div>
          </div> -->
      </div>
      <!-- table -->
      <div class="row">
        <div class="col-md-12">
      <div class="w-100 talent_overflowx mt-3">
          <table class="table table-bordered table-responsive-md talent_table_padding">
              <thead>
                  <tr>
                      <th scope="col" class="text-center">
                          <a href="#" class="talent_sort" style="color: white;">
                            Customer Type
                          </a>
                      </th>
                      <th scope="col" class="text-center">
                          <a href="#" class="talent_sort" style="color: white;">
                            Umur
                          </a>
                      </th>
                      <th scope="col" class="text-center" style="max-width: 150px">
                          <a href="#" class="talent_sort" style="color: white;">
                            Status
                          </a>
                      </th>
                      <th scope="col" class="text-center" style="max-width: 150px">
                          <a href="#" class="talent_sort" style="color: white;">
                            Pekerjaan
                          </a>
                      </th>
                      <th scope="col" class="text-center" style="max-width: 150px">
                          <a href="#" class="talent_sort" style="color: white;">
                            Penghasilan
                          </a>
                      </th>
                      <th scope="col" class="text-center" style="max-width: 150px">
                          <a href="#" class="talent_sort" style="color: white;">
                            Kendaraan Sebelumnya
                          </a>
                      </th>
                      <th scope="col" class="text-center" style="max-width: 150px">
                          <a href="#" class="talent_sort" style="color: white;">
                            Sumber Prospek
                          </a>
                      </th>
                      <th scope="col" class="text-center" style="max-width: 150px">
                          <a href="#" class="talent_sort" style="color: white;">
                            Published
                          </a>
                      </th>
                      <th scope="col" class="text-center" colspan="2">
                          <a href="#" class="talent_sort" style="color: white;">
                            Action
                          </a>
                      </th>
                  </tr>
              </thead>
              <tbody>
                  <tr v-for="dictionary in productTypes" :key="dictionary.productTypeId">
                      <td>
                          {{dictionary.productCustomerTypeName}}
                      </td>
                      <td>
                          {{dictionary.productCustomerUmur}}
                      </td>
                      <td>
                          {{dictionary.productCustomerStatus}}
                      </td>
                      <td>
                          {{dictionary.productCustomerPekerjaan}}
                      </td>
                      <td>
                          {{dictionary.productCustomerPenghasilan}}
                      </td>
                      <td>
                          {{dictionary.productCustomerPrevVehicle}}
                      </td>
                      <td>
                          {{dictionary.productCustomerProspectSource}}
                      </td>
                      <td>
                          <button
                            :class="`btn btn-block ${dictionary.approvedAt ? 'talent_bg_green talent_font_white' : 'talent_bg_red talent_font_white'}`" 
                            @click="handleEditStatus(dictionary)">
                              <fa :icon="dictionary.approvedAt ? 'eye' : 'eye-slash'"></fa>
                          </button>
                      </td>
                      <td class="talent_nopadding_important">
                          <button class="btn btn-block talent_bg_cyan talent_font_white" @click="handleEdit(dictionary)">Edit</button>
                      </td>
                      <td class="talent_nopadding_important">
                        <button type="button" class="btn btn-block talent_bg_red talent_font_white" 
                                  data-toggle="modal"
                                  data-target="#modalDelete" @click="selectedData = dictionary">
                          Remove
                        </button>
                      </td>
                  </tr>
              </tbody>
          </table>
      </div>

      <!--Pagination-->
      <div class="col-md-12 d-flex justify-content-center">
          <paginate :currentPage.sync="params.PageIndex" :total-data="totalData" :limit-data="10" @update:currentPage="fetchFAQ()"></paginate>
      </div>
        </div>
      </div>
    </div>


    <modal-delete
      name="modalDelete"
      @delete="handleDelete"
    />
    <modal-customer-type name="modalCustomerType"/>
  </div>
</template>


<script>
import { ProductInformationService } from '../../../services/ProductInformation/ProductInformationService';
import $ from 'jquery'
import ModalDelete from '../../shared/ModalDelete.vue';
import moment from 'moment'
import ModalCustomerType from '../ModalCustomerType.vue';

export default {
  components: { ModalDelete, ModalCustomerType },
    async mounted() {
      this.isBusy = true
      try {
        await this.fetchCategories()
        await this.fetchFAQ()
      } catch(err) {
        console.error(err)
      }
      this.isBusy = false

      const vm = this
      $('#modalCustomerType').on('hidden.bs.modal', function (e) {
        vm.fetchCategories()
      })
    },
    data() {
      return {
        moment: moment,
        isBusy: false,
        errored: [],
        selectedFile: null,
        productTypes: [],
        categories: [],
        totalData: 0,
        selectedCategory: null,
        service: new ProductInformationService(),
        params: {
          ProductId: '',
          SortBy: '',
          PageIndex: 1,
          PageSize: 10
        },
        paramsCategories: {
          ProductFaqCategoryName: "",
          SortBy: '',
          PageIndex: 1,
          PageSize: 100
        },
        name: '',
        salesTalk: '',
        image: {
          base64: '',
          fileName: '',
          contentType: ''
        },
        success: false,
        selectedData: null,
        selectedEditData: null,
        status : '',
        kebutuhan : '',
        penghasilan : '',
        umur : '',
        pekerjaan : '',
        kendaraanSebelumnya : '',
        sumberProspek : ''
      }
    },
    methods: {
      handleAddSub() {
        $('#modalCustomerType').modal('show')
      },
      reset() {
          this.success = false
          this.status = ''
          this.kebutuhan = ''
          this.penghasilan = ''
          this.umur = ''
          this.pekerjaan = ''
          this.kendaraanSebelumnya = ''
          this.sumberProspek = ''
          this.selectedCategory = null
          this.selectedEditData = null
          window.location.href = `/MyTools/ProductInformation`
      },
      async fetchCategories() {
        try {
          const res = await this.service.getAllCustomerType(this.paramsCategories)
          this.categories = res.productCustomerTypes
          this.totalData = res.totalProductCustomerTypes
        } catch(err) {
          console.error(err)
        }
      },
      async fetchFAQ() {
        this.isBusy = true
        try {
          this.params.ProductId = this.id
          const res = await this.service.getAllCustomer(this.params)
          this.productTypes = res.productCustomers
        } catch(err) {
          console.error(err)
        }
        this.isBusy = false
      },
      async submit(e) {
        if(this.handleValidation()) {
          return
        }
        this.isBusy = true
        this.success = false
        try {
          const body = {
            productId: this.id,
            "productCustomerTypeId": this.selectedCategory.productCustomerTypeId,
            "productCustomerStatus": this.status,
            "productCustomerKebutuhan": this.kebutuhan,
            "productCustomerPenghasilan": this.penghasilan,
            "productCustomerUmur": this.umur,
            "productCustomerPekerjaan": this.pekerjaan,
            "productCustomerPrevVehicle": this.kendaraanSebelumnya,
            "productCustomerProspectSource": this.sumberProspek,
            approvedAt: e ? moment().format('YYYY-MM-DD') : null
          }
          if(!this.selectedEditData) {
            await this.service.createCustomer(body)
          } else {
            if(!this.image.base64) {
              delete body.productGalleryFileContent
            }
            body.productCustomerId = this.selectedEditData.productCustomerId
            await this.service.updateCustomer(body)
          }
          this.params.PageIndex = 1
          this.fetchFAQ()
          this.success = true
          this.status = ''
          this.kebutuhan = ''
          this.penghasilan = ''
          this.umur = ''
          this.pekerjaan = ''
          this.kendaraanSebelumnya = ''
          this.sumberProspek = ''
          this.selectedCategory = null
          this.selectedEditData = null
        } catch (err) {
          console.error(err)
        }
        this.isBusy = false
      },
      handleValidation () {
        this.errored = []
        if(!this.selectedCategory) {
          this.errored.push({
            key: 'category',
            value: 'Please fill category field'
          })
        }
        if(!this.umur) {
          this.errored.push({
            key: 'umur',
            value: 'Please fill umur field'
          })
        }
        if(!this.status) {
          this.errored.push({
            key: 'status',
            value: 'Please fill status field'
          })
        }
        if(!this.pekerjaan) {
          this.errored.push({
            key: 'pekerjaan',
            value: 'Please fill pekerjaan field'
          })
        }
        if(!this.kendaraanSebelumnya) {
          this.errored.push({
            key: 'kendaraan sebelumnya',
            value: 'Please fill kendaraan sebelumnya field'
          })
        }
        return this.errored.length > 0
      },
      handleFilter() {
        this.params.PageIndex = 1
        this.fetchFAQ()
      },
      async handleDelete() {
        this.isBusy = true
        try {
          await this.service.deleteCustomer(this.selectedData.productCustomerId)
          this.fetchFAQ()
          this.success = true
          this.status = ''
          this.kebutuhan = ''
          this.penghasilan = ''
          this.umur = ''
          this.pekerjaan = ''
          this.kendaraanSebelumnya = ''
          this.sumberProspek = ''
          this.selectedCategory = null
          this.selectedEditData = null
        } catch (err) {
          console.error(err)
        }
        this.isBusy = false
      },
      handleEdit(data) {
        this.selectedEditData = data
        this.status = data.productCustomerStatus
        this.kebutuhan = data.productCustomerKebutuhan
        this.penghasilan = data.productCustomerPenghasilan
        this.umur = data.productCustomerUmur
        this.pekerjaan = data.productCustomerPekerjaan
        this.kendaraanSebelumnya = data.productCustomerPrevVehicle
        this.sumberProspek = data.productCustomerProspectSource
        this.selectedCategory = {
          productCustomerTypeId: data.productCustomerTypeId,
          productCustomerTypeName: data.productCustomerTypeName
        }
      },
      async handleEditStatus(data) {
        this.isBusy = true
        await this.service.updateCustomer({
          productCustomerId: data.productCustomerId,
          productId: data.productId,
          "productCustomerTypeId": data.productCustomerTypeId,
          "productCustomerStatus": data.productCustomerStatus,
          "productCustomerKebutuhan": data.productCustomerKebutuhan,
          "productCustomerPenghasilan": data.productCustomerPenghasilan,
          "productCustomerUmur": data.productCustomerUmur,
          "productCustomerPekerjaan": data.productCustomerPekerjaan,
          "productCustomerPrevVehicle": data.productCustomerPrevVehicle,
          "productCustomerProspectSource": data.productCustomerProspectSource,
          approvedAt: data.approvedAt ? null : moment().format('YYYY-MM-DD')
        })
        this.fetchFAQ()
      },
      async handleChangeFile(e) {
        const selectedImage = e.target.files[0]
        // if(selectedImage.size > 104857600) {
        //     this.image.base64 = ''
        //     this.image.contentType = ''
        //     this.image.fileName = ''
        //     return
        // }
        let array = selectedImage.name.split(".");
        let extension = array.pop();
        const res = await this.getBase64(selectedImage)
        this.image.base64 = res
        this.image.fileName = selectedImage.name
        this.image.contentType = extension
      },
      getBase64(file) {
        return new Promise((resolve, reject) => {
            const reader = new FileReader();
            reader.readAsDataURL(file);
            reader.onload = () => {
            let encoded = reader.result.toString().replace(/^data:(.*,)?/, '');
            if ((encoded.length % 4) > 0) {
                encoded += '='.repeat(4 - (encoded.length % 4));
            }
            this.selectedFile = reader.result
            resolve(encoded);
            };
            reader.onerror = error => reject(error);
        });
      }
    },
    computed: {
      id() {
        let uri = window.location.search.substring(1); 
        let params = new URLSearchParams(uri);
        return params.get("id")
      }
    }
}
</script>

<style scoped>
.container-upload {
  width: 100%;
  border: 1px solid #000000;
  background-color: rgb(146, 145, 145);
  position: relative;
  display: flex;
}
.btn-upload {
  position: absolute;
  bottom: 0;
  display: flex;
  justify-content: center;
  width: 100%;
  padding: 16px 0px;
}
</style>