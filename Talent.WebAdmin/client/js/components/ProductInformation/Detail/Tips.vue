<template>
  <div class="row px-4 py-4 w-100">
          ...
          <div class="input-group talent_front">
            ...
            <div class="d-flex align-items-center w-100">
              <multiselect
                ...
                name="Category"
                :options="categories"
                label="productTipCategoryName"
                track-by="productTipCategoryId"
          ...
          <div class="input-group talent_front">
            ...
            <div class="d-flex align-items-center w-100">
              <div class="container-upload">
                ...
                <!-- </div> -->
                  <input type="file" name="input-manfaat" id="input-manfaat" style="display: none;" @change="handleChangeFile" accept=".png,.jpg,.jpeg,.mp4,.pdf,.pdf">
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="col-md-6">
      <div class="row">
        <div class="col-md-12">
          ...
          <div class="input-group">
              <input name="title" type="text" class="form-control" v-model="topic"/>
          </div>
        </div>
        <div class="col-md-12 mt-4">
          <div class="input-group talent_front">
    ...
    <div class="col-md-12 mt-4">
      <div style="height: 500px;" v-if="image.contentType === 'pdf'">
        <object v-if="image.contentType === 'pdf'" class="w-100" type="application/pdf" width="100%" height="500px" :data="selectedFile ? selectedFile : `/upload-image2.png`" for="customFileFile" style="height: 100%"/>
      </div>
    </div>
    ...
    <div class="col-md-12" v-if="!selectedEditData">
      ...
      <div class="col-md-12 talent_magin_small mt-3">
          <div class="row align-items-center justify-content-between">
              <div class="col d-flex py-0 pl-0 pr-4">
                  ...
                  <div class="input-group">
                      <input class="form-control" placeholder="Search By Title" v-model="params.ProductTipTitle" @click="handleFilter"/>
                  </div>
              </div>
      ...
      <div class="w-100 talent_overflowx mt-3">
          <table class="table table-bordered table-responsive-md talent_table_padding">
              ...
              <tbody>
                  <tr v-for="dictionary in productTypes" :key="dictionary.productTipId">
                      <td>
                          {{dictionary.productTipCategoryName}}
                      </td>
                      <td>
                          {{dictionary.productTipTitle}}
                      </td>
                      <td style="max-width: 150px; min-height: 100px; height: 100%" v-html="dictionary.productTipDescription">
                      </td>
                      <td>
                      ...
                      <td class="talent_nopadding_important text-center">
                        <div class="d-flex align-items-center w-100">
                          ...
                          <button type="button" class="btn btn-block talent_bg_orange" v-if="dictionary.isApproved == 'Draft'" @click="handleChangeStatus(dictionary)">
                            Approve
                          </button>
                        </div>
                      </td>
...
export default {
  components: { ModalDelete, ModalCategoryTip },
    async mounted() {
      this.isBusy = true
      try {
      ...
      const vm = this
      $('#modalCategoryTip').on('hidden.bs.modal', function (e) {
        vm.fetchCategories()
      })
    },
    data() {
    ...
    methods: {
      ...
      async fetchCategories() {
        try {
          const res = await this.service.getAllTipCategory({
            ProductId: this.id,
          SortBy: '',
          PageIndex: 1,
          PageSize: 100
      ...
      async fetchFAQ() {
        ...
        try {
          ...
          const res = await this.service.getAllTip(this.params)
          this.productTypes = res.productTips
          this.totalData = res.totalProductTips
        } catch(err) {
          console.error(err)
        }
        this.isBusy = false
      ...
      async submit(e) {
        ...
        try {
          const body = {
            productId: this.id,
            productTipCategoryId: this.selectedCategory.productTipCategoryId,
          ...
          "productTipTitle": this.topic,
          "productTipDescription": this.description,
          approvedAt: e ?moment().format('YYYY-MM-DD')  : null
          }
          if(!this.selectedEditData) {
            await this.service.createTip(body)
          } else {
            if(!this.image.base64) {
              delete body.productGalleryFileContent
            }
            body.productTipId = this.selectedEditData.productTipId
            await this.service.updateTip(body)
          }
      ...
      async handleDelete() {
        this.isBusy = true
        try {
          await this.service.deleteTip(this.selectedData.productTipId)
          this.fetchFAQ()
          this.success = true
          this.sequence = 1
          this.topic = ''
      ...
      handleEdit(data) {
        ...
        this.topic = data.productTipTitle
        this.description = data.productTipDescription
        this.sequence = data.isSequence
        this.selectedCategory = {
          productTipCategoryId: data.productTipCategoryId,
          productTipCategoryName: data.productTipCategoryName
        }
      ...
      async handleEditStatus(data) {
        this.isBusy = true
        await this.service.updateTip({
          productTipId: data.productTipId,
          productId: data.productId,
          productTipCategoryId: data.productTipCategoryId,
          "isSequence": data.isSequence,
          "productTipTitle": data.productTipTitle,
          "productTipDescription": data.productTipDescription,
      ...
      async handleChangeStatus(data) {
        this.isBusy = true
        await this.service.updateApprovalTip({
          "productTipId": data.productTipId,
          "isApproved": data.isApproved == 'Draft' ? "Published" : 'Draft',
          "isUpdateProductTipApproval": true
        })
        this.fetchFAQ()
      },
