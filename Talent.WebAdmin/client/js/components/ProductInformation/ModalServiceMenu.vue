            ...
            <div class="modal-content">
                ...
                <div class="col-md-12 talent_magin_small mt-3">
                    <div class="row align-items-center justify-content-between">
                        <div class="col d-flex py-0 pl-0 pr-4">
                            ...
                            <div class="input-group">
                                <input class="form-control" placeholder="Search By Name" v-model="params.ServiceTipMenuName"/>
                            </div>
                        </div>
                ...
                <div class="w-100 talent_overflowx mt-3">
                    <table class="table table-bordered table-responsive-md talent_table_padding">
                        ...
                        <tbody>
                            <tr v-for="dictionary in categories" :key="dictionary.serviceTipMenuId">
                                <td>
  ...
  components: { ModalDelete },
    ...
    methods: {
      async fetchCategories() {
        this.params.Cms_MenuCategory = this.type
        try {
            const res = await this.service.getAllServiceTipsMenu(this.params)
            this.categories = res.serviceTipMenus
            this.totalData = res.totalServiceTipMenus
        } catch (err) {
            console.error(err)
        }
      },
      async handleSave() {
        ...
        try {
          if(this.selectedEditData) {
            await this.service.updateServiceTipsMenu({
              serviceTipMenuId: this.selectedEditData.serviceTipMenuId,
              "serviceTipMenuName": this.categoryName,
              "isSequence": 0
            })

          } else {
            await this.service.createServiceTipsMenu({
              "serviceTipMenuName": this.categoryName,
              "isSequence": 0
            })
          }
      ...
      async handleDelete() {
        this.isBusy = true
        try {
          await this.service.deleteServiceTipsMenu(this.selectedData.serviceTipMenuId)
          this.fetchCategories()
          this.success = true
          this.categoryName = ''
        } catch (err) {
      ...
      handleEdit(category) {
        ...
        this.categoryName = category.serviceTipMenuName
      }
    }
}
</script>
