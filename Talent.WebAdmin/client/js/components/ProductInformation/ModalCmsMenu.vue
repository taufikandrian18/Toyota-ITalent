            ...
            <div class="modal-content">
                <div class="modal-header">
                    <div class="col-md-12 d-flex">
                        Add {{ !['KPOM', 'Kodawari'].includes(type) ? 'Menu' : 'Category' }}
                    </div>
                </div>
                <div class="row px-4 py-4">
                  ...
                  <div class="col-md-6">
                    <div class="row">
                      <div class="col-md-12">
                        <label>{{ !['KPOM', 'Kodawari'].includes(type) ? 'Menu' : 'Category' }} Name<span class="talent_font_red"> *</span></label>
                ...
                <div class="col-md-12 talent_magin_small mt-3">
                    <div class="row align-items-center justify-content-between">
                        <div class="col d-flex py-0 pl-0 pr-4">
                            ...
                            <div class="input-group">
                                <input class="form-control" placeholder="Search By Name" v-model="params.Cms_MenuName"/>
                            </div>
                        </div>
                ...
                <div class="w-100 talent_overflowx mt-3">
                    <table class="table table-bordered table-responsive-md talent_table_padding">
                        ...
                        <tbody>
                            <tr v-for="dictionary in categories" :key="dictionary.cms_MenuId">
                                <td>
                                ...
                                <td class="talent_nopadding_important">
                                    <button type="button" class="btn btn-block talent_bg_red talent_font_white" 
                                            data-toggle="modal"
                                            data-target="#modalDeleteCmsMenu" @click="selectedData = dictionary">
                                    Remove
                                    </button>
                                </td>
                            </tr>
  ...
  components: { ModalDelete },
    ...
    methods: {
      async fetchCategories() {
        this.params.Cms_MenuCategory = this.type
        try {
            const res = await this.service.getAllCMSMenu(this.params)
            this.categories = res.cmsMenus
            this.totalData = res.totalCmsMenus
        } catch (err) {
            console.error(err)
        }
      },
      async handleSave() {
        ...
        try {
          if(this.selectedEditData) {
            await this.service.updateCMSMenu({
              cms_MenuId: this.selectedEditData.cms_MenuId,
              "cms_MenuName": this.categoryName,
              "cms_MenuCategory": this.type
            })

          } else {
            await this.service.createCMSMenu({
              "cms_MenuName": this.categoryName,
              "cms_MenuCategory": this.type
            })
          }
      ...
      async handleDelete() {
        this.isBusy = true
        try {
          await this.service.deleteCMSMenu(this.selectedData.cms_MenuId)
          this.fetchCategories()
          this.success = true
          this.categoryName = ''
        } catch (err) {
      ...
      handleEdit(category) {
        ...
        this.categoryName = category.cms_MenuName
      }
    }
}
</script>
