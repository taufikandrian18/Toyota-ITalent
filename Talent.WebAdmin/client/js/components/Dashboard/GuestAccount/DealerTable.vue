<template>
  <div>
    ...
    <div class="table-container w-100">
      <table
          class="table table-hover table-bordered table-responsive w-100"
      >
...
export default {
  ...
  methods: {
    ...
    async fetchSummary () {
      const params = this.selectedDealers.map(v => `id=${v.dealerId}&`)
      try {
        const res = await this.accountService.getSummaryDealer(params.length > 0 ? params : '')
