      ...
      <div class="row">
        <div class="col-md-6">
          <div class="row">
            ...
            <div class="col-md-12 mt-4" v-if="selectedKodawariMenu">
              <div class="input-group talent_front">
              ...
              <div class="input-group talent_front">
                ...
                <div class="d-flex align-items-center w-100">
                  <multiselect
                    ...
                    name="Category"
                    :options="categories"
                    label="kodawariTypeName"
                    track-by="kodawariTypeId"
      ...
      <div class="col-md-12 talent_magin_small mt-3">
          <div class="row align-items-center justify-content-between">
              <div class="col d-flex py-0 pl-0 pr-4">
                  ...
                  <div class="input-group">
                      <input class="form-control" placeholder="Search By Menu" v-model="params.KodawariMenuName" @keyup.enter="handleFilter"/>
                  </div>
              </div>
...
export default {
  components: { ModalDelete, ModalCmsMenu, ModalSpwaCategory, ModalCmsContentFull, ModalCmsSubContent, ModalKodawariMenu, ModalKodawariType },
    async mounted() {
    this.isBusy = true
      try {
    ...
    methods: {
      ...
      async fetchKodawariMenu() {
        try {
          ...
          this.kodawariMenus = res.kodawariMenus
        } catch(err) {
          console.error(err)
        }
      },
      async fetchCmsMenu() {
        try {
          const res = await this.service.getAllCMSMenu({
            Cms_menuCategory: this.kodawariMenu,
            PageIndex: 1,
            PageSize: 100
          })
      ...
      async fetchCmsSubContent() {
        try {
          const res = await this.service.getAllCMSSubContent({
            Cms_SubContentCategory: 'Kodawari',
            PageIndex: 1,
            PageSize: 100
          })
      ...
      async fetchCategories() {
        try {
          const res = await this.service.getAllKodawariType(this.paramsCategories)
      ...
      async fetchFAQ() {
        ...
        try {
          const res = await this.service.getAllKodawari(this.params)
          this.productTypes = res.kodawaris
          this.totalData = res.totalKodawaris
        } catch(err) {
          console.error(err)
        }
      ...
      async submit(e) {
        ...
        try {
          const body = {
            "kodawariTypeId": this.selectedCategory.kodawariTypeId,
            "kodawariSequence": this.sequence,
            "cms_ContentId": this.selectedCmsContent.cms_ContentId,
            "cms_MenuId": this.selectedCmsMenu.cms_MenuId,
            "cms_SubContentId": this.selectedSubCmsContent ? this.selectedSubCmsContent.cms_SubContentId : null,
            "kodawariMenuId": this.selectedKodawariMenu.kodawariMenuId,
            approvedAt: e ?moment().format('YYYY-MM-DD')  : null
          }
          if(!this.selectedEditData) {
            await this.service.createKodawari(body)
          } else {
            body.kodawariId = this.selectedEditData.kodawariId
            await this.service.updateKodawari(body)
          }
      ...
      async handleDelete() {
        this.isBusy = true
        try {
          await this.service.deleteKodawari(this.selectedData.kodawariId)
          this.fetchFAQ()
          this.success = true
          this.sequence = 1
          this.topic = ''
      ...
      handleEdit(data) {
        ...
        this.sequence = data.kodawariSequence
        this.selectedCategory = {
          kodawariTypeId: data.kodawariTypeId,
          kodawariTypeName: data.kodawariTypeName
        }
      ...
      async handleEditStatus(data) {
        this.isBusy = true
        await this.service.updateKodawari({
          kodawariId: data.kodawariId,
          "kodawariTypeId": data.kodawariTypeId,
          "kodawariSequence": data.kodawariSequence,
          "cms_ContentId": data.cms_ContentId,
          "cms_MenuId": data.cms_MenuId,
          "cms_SubContentId": data.cms_SubContentId,
          "kodawariMenuId": data.kodawariMenuId,
    ...
    watch: {
      selectedKodawariMenu() {
        if(this.selectedKodawariMenu) {
          console.log('called')
          this.kodawariMenu = `Kodawari_${this.selectedKodawariMenu.kodawariMenuName}`
          this.fetchCmsMenu()
        } else {
          this.kodawariMenu = 'Kodawari'
        }
      }
    }
}
